...
This guide covers setting up Stripe for GoSaaS in **standalone mode** (without Levee).

## Overview

GoSaaS supports two modes for billing:
...
Products are defined in `etc/gosaas.yaml`. The app syncs them to Stripe on startup.
...
1. Reads products from `etc/gosaas.yaml`
...
- Products: `gosaas_slug: "pro"`
- Prices: `gosaas_key: "pro-monthly"`
...
2. Products are defined in `etc/gosaas.yaml`
...
4. [ ] Review pricing in `etc/gosaas.yaml`

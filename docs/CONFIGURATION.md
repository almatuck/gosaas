...
Complete reference for all configuration in GoSaaS.
...
| `etc/gosaas.yaml` | Backend config + products | Yes | Change pricing, features |
...
## 2. Backend Configuration (gosaas.yaml)

Backend settings and product/pricing definitions.

**Location:** `etc/gosaas.yaml`
...
1. Products/prices are defined in `etc/gosaas.yaml`
...
```
etc/gosaas.yaml → pnpm build → pricing.html (static)

...
GoSaaS deploys as a **single binary with automatic SSL** - no nginx, Apache, or reverse proxy required.
...
GoSaaS handles everything internally:
...
# Result: single binary at ./bin/gosaas
```
...
```bash
# On your server
scp ./bin/gosaas .env user@server:/app/
ssh user@server
cd /app
./gosaas
```
...
Create `/etc/systemd/system/gosaas.service`:

```ini
[Unit]
Description=GoSaaS Application
...
sudo systemctl enable gosaas
sudo systemctl start gosaas
sudo journalctl -u gosaas -f  # View logs
```
...
sudo setcap 'cap_net_bind_service=+ep' ./gosaas
...
2. Check server is running: `ps aux | grep gosaas`

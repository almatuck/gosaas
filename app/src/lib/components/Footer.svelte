<script lang="ts">
	...
	// Build social links from site config - only show if URL is set
	type SocialLink = { label: string; href: string; icon: string };
	const socialConfig: Array<{ key: keyof typeof site.social; label: string; icon: string }> = [
		{ key: 'twitter', label: 'Twitter', icon: 'twitter' },
		{ key: 'linkedin', label: 'LinkedIn', icon: 'linkedin' },
		{ key: 'github', label: 'GitHub', icon: 'github' },
		{ key: 'discord', label: 'Discord', icon: 'discord' }
	];
	const socialLinks: SocialLink[] = socialConfig
		.filter((s) => site.social[s.key])
		.map((s) => ({ label: s.label, href: site.social[s.key], icon: s.icon }));
	...
	const footerLinks = {
		...
		legal: [
			...
			{ label: 'Terms of Service', href: site.legal.termsUrl }
		]
	};
</script>

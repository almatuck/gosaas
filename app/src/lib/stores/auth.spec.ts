...
describe('Auth Store', () => {
	...
	describe('logout', () => {
		it('should reset state to initial values', async () => {
			...
			const mockApi = await import('$lib/api/gosaas');
		...
		it('should clear localStorage on logout', () => {
			localStorageMock.setItem('gosaas_token', 'test');
			localStorageMock.setItem('gosaas_refresh_token', 'test');
			localStorageMock.setItem('gosaas_expires_at', '123456');

			auth.logout();

			expect(localStorageMock.removeItem).toHaveBeenCalledWith('gosaas_token');
			expect(localStorageMock.removeItem).toHaveBeenCalledWith('gosaas_refresh_token');
			expect(localStorageMock.removeItem).toHaveBeenCalledWith('gosaas_expires_at');
		});
	});

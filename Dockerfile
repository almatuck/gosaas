# Multi-stage Dockerfile for GoSaaS
# Single all-in-one binary with embedded SvelteKit frontend
# Usage: docker build -t gosaas .
...
# Copy gosaas.yaml for pricing (read at build time by +page.server.ts)
COPY etc/gosaas.yaml ../etc/gosaas.yaml
...
COPY --from=builder /app/bin/gosaas ./gosaas

...
// ForgotPasswordHandler - Request password reset
...
func (l *ForgotPasswordLogic) ForgotPassword(req *types.ForgotPasswordRequest) (resp *types.MessageResponse, err error) {
	...
	if err != nil {
		slog.Error("forgot password failed", "email", req.Email, "error", err)
...
<body style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
	...
	<p style="color: #666; font-size: 14px;">If you didn't request this, you can safely ignore this email.</p>
		...
		if emailErr != nil {
			slog.Error("failed to send password reset email", "error", emailErr)
		} else {

...
GoSaaS uses a simple 3-file configuration system:

| File | Purpose | When to Edit |
|------|---------|--------------|
| `app/src/lib/config/site.ts` | Frontend branding, SEO, social links | Rebrand your product |
| `etc/gosaas.yaml` | Backend config + products/pricing | Add features, change pricing |
...
### 2. Products & Pricing (gosaas.yaml)

Edit `etc/gosaas.yaml` to configure your pricing plans:
...
- **Products**: Synced to Stripe on startup from `gosaas.yaml`
...
GoSaaS deploys as a single binary - **no nginx, Apache, or reverse proxy needed**:

```bash
make build
cd app && pnpm build
# Deploy ./bin/gosaas + .env anywhere
./bin/gosaas
```
